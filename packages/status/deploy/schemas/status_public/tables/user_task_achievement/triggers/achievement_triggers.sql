-- Deploy schemas/status_public/tables/user_task_achievement/triggers/achievement_triggers to pg
...
-- CREATE FUNCTION status_private.tg_achievement_using_field ()
--   RETURNS TRIGGER
--   AS $$
-- DECLARE
--   is_null boolean;
...
--         IF (is_null IS FALSE) THEN
--             PERFORM status_private.user_completed_task(task_name, user_id);
--         END IF;
--         RETURN NEW;
--     END IF;
-- END;
...
-- CREATE FUNCTION status_private.tg_achievement_toggle_using_field ()
--   RETURNS TRIGGER
--   AS $$
-- DECLARE
--   is_null boolean;
...
--         USING NEW INTO user_id;
--         IF (is_null IS TRUE) THEN
--             PERFORM status_private.user_incompleted_task(task_name, user_id);
--         ELSE
--             PERFORM status_private.user_completed_task(task_name, user_id);
--         END IF;
--         RETURN NEW;
