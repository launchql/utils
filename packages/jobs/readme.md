...
<p align="center" width="100%">
  <a href="https://github.com/launchql/utils/actions/workflows/run-tests.yaml">
   ...
   <a href="https://www.npmjs.com/package/@launchql/ext-jobs"><img height="20" src="https://img.shields.io/github/package-json/v/launchql/utils?filename=packages%2Fjobs%2Fpackage.json"/></a>
</p>

An asynchronous job queue schema for ACID compliant job creation through triggers/functions/etc. This PostgreSQL extension provides a robust job queue system for background processing.

## Usage

```sql
-- Create a job
SELECT jobs.add_job('my_job_type', json_build_object('key', 'value'));

-- Process jobs
SELECT jobs.process_jobs();
...
-- Query job status
SELECT * FROM jobs.jobs WHERE job_type = 'send_email';
```
